...
- `ChatComposerView` in LodyKit owns the shared native input, draft handoff/restore, and model controls. Chat embeds it directly; new-session sheets use `NativeComposer` with React Native keyboard avoidance and native height events. Creation remains text-only, so its host hides the attachment picker.
