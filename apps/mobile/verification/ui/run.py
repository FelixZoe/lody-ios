...
metro = subprocess.Popen(['pnpm', '--filter', '@lody-ios/mobile', 'exec', 'expo', 'start', '--dev-client', '--host', 'lan', '--port', str(args.port)],
                         cwd=ROOT, env={**os.environ, 'CI': '1', 'EXPO_NO_DOTENV': '1', 'EXPO_PUBLIC_UI_VERIFY': '1', 'REACT_NATIVE_PACKAGER_HOSTNAME': '127.0.0.1'},
    ...
    for appearance in ['light', 'dark']:
        ...
        for case in cases:
            ...
            try:
                ...
                ui.axe('tap', '--id', 'composer-preview' if case == 'composer' else case if case.startswith('composer-') else 'chat-preview', '--pre-delay', '0.8', '--post-delay', '0.8')
                ui.element('create-session-input' if case == 'composer' else 'session-input' if case.startswith('composer-') else 'chat-navigation-title')
